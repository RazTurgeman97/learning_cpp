...
float StudentRecords::get_GPA(int sid) const{ // const correctness
    float points = 0.0f, credits = 0.0f;
    for (const Grade& grd : grades)  // the type of GRD was referenced to grade. since we are inside a getter function that enforces const correctness,
    // this type is in conflict. that constant qualifier at the end of line 88 means that the code inside the function cannot modify this student records object.
    //  So to solve this inconsistency, we can pledge never to modify GRD by simply adding a const qualifier to it.
        if (grd.get_student_id() == sid){
            unsigned char current_credits = get_course_credits(grd.get_course_id());
            credits += current_credits;
            points += get_num_grade(grd.get_grade()) * current_credits; // used 'get_num_grade' instead of the original num_grade variable.
        }
